...
void Player_Deck::createDeck(const std::weak_ptr<Opponent_Field>&field)
{
	...
	for (unsigned int i = 0; i <deck.size(); i++)
	{
		...
		if (deck.at(i).at(0) == 'm')
		{
			...
			mCard.push_back(std::shared_ptr<BaseCard>(new MonsterCard(y, 1)));
		...
		else if (deck.at(i).at(0) == 'e')
		{
			...
			mCard.push_back(std::shared_ptr<BaseCard>(new EffectCard(y)));
			mCard[i]->setPos(mPosX, mPosY);
		...
		mCard[i]->changePlacePosition(Position::DECK);
	...
	{
		if (deck.at(i).at(0) == 'm') dynamic_pointer_cast<MonsterCard>(mCard[i])->loadTexture();
	}
	//-----------------------------------------
	/*
	deck = lD.loadMonster();
	...
	{
		mCard.push_back(std::shared_ptr<BaseCard>(new MonsterCard(deck.at(i), 1)));
		mCard[i]->setPos(mPosX, mPosY);
	...
	{
		dynamic_pointer_cast<MonsterCard>(mCard[i])->loadTexture();
	}
	...
	for (int i = 0; i < deck.size(); i++)
	{
		mCard.push_back(std::shared_ptr<BaseCard>(new EffectCard(deck.at(i))));
		...
		if (mField != nullptr && mHand != nullptr)
		{
			...
			mCard[mCard.size() - 1]->changePlacePosition(Position::DECK);
		}
